...
export function CarModelPage() {
  ...
  const confirmDelete = async () => {
    const { data, error } = await onDelete(
      "car_model",
      selectedCar?.id as number
    );
    if (data) {
      toast.success(`${selectedCar?.make}(${selectedCar?.model}) deleted`);
      setOpenedModal("");
      refresh();
    }
  ...
  const {
    ...
    refresh,
    count,
  } = useQuery<CarModel[]>({ table: "car_model", from: start, to: end });

  const onSubmit: SubmitHandler<CarModel> = async (values) => {
    const { data, error } = selectedCar
      ? await update("car_model", selectedCar.id, values)
  ...
  const onSearch = async (text: string) => {
    if (text.length > 0) search(`make.ilike.%${text}%,model.ilike.%${text}%`);
  };
